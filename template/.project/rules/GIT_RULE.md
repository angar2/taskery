...
- **검수 기동 (자동)**: `/task-dev`·`/task-test`가 *이번 세션의 마지막 단계(중단점)*를 끝낼 때 위 포트로 서버를 백그라운드 기동하고 접속 URL을 보고한다 (사용자가 매번 실행을 지시하지 않게). dev→test로 계속 진행하면 중간 단계에선 띄우지 않고 마지막 단계에서만. 단계가 *기능상* 서버를 요구하면(E2E 등) 중단점과 무관하게 띄운다.
...
| 2026-06-24 | §멀티세션 워크트리 정책에 "멀티세션 검수 환경" 추가 — task별 dev 서버 포트 격리(메인=기준 포트, task=기준+TASK번호) + 터널 독립 + close 시 자기 것만 종료(광역 종료 금지) + 중단점 검수 서버 자동 기동. FRICTION 검토 F7 반영 |
