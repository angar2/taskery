...
`draft` 상태 task의 본문 4 섹션(Requirements / Scope / Dev Plan / Test Plan)을 채우고 status를 `planned`로 갱신. 사용자와 대화로 진행 — 인터뷰 + 증폭 + 코드 서치 + Phase 분할. 해석 여지가 있는 태스크는 Test Plan 출제를 격리 서브에이전트 B에 분리한다 (Step 2.5 — 구현자 겸 출제의 오독 동조 차단).
...
3. 상태 검증 — 두 진입만 허용, 그 외 종료 + 안내:
   ...
   - **`testing` + Test Plan 보수 모드** → `/task-test`가 *시험문제 결함*으로 반려해 되돌아온 경우에 한해 허용. 이때는 **Test Plan 섹션만 보수**(Requirements / Scope / Dev Plan 보존), Step 2~4는 건너뛰고 Step 5(Test Plan)만 수행, status는 `testing` 그대로 둔다(전이 X — 보수 후 `/task-test` 재실행). **origin 파일(`<NNN>_<slug>_test-plan-origin.md`)이 있는 태스크는** 보수로 바뀐 칸마다 `【보수 · 원안: "<원문>"】` 플래그를 단다 — 무플래그 차이는 task-test가 오염으로 반려한다.
...
### Step 2.5 — 출제 분리 판단 (Test Plan 작성 주체 결정)

출제(Test Plan)를 구현자(A=메인)가 겸하면 요구사항을 오독했을 때 *같은 오독이 문제지와 답안지에 함께* 들어가 검증이 오독을 잡지 못한다 (recordion FRICTION 2026-08-09). 그래서 해석 여지가 있는 태스크는 Test Plan을 **격리 서브에이전트 B**에 위임한다.

**적용 조건** — 다음 둘 다 해당할 때만:
1. 헤더 유형이 `feature` / `improvement` / `bug`
2. Requirements에 해석 여지가 있다 (UX/UI·기능 동작 — "어떻게 보여야/움직여야 하나"가 자명하지 않은 태스크)

`chore`·`docs`·오타·설정 등 해석 여지 없는 태스크는 **분리 비적용** — 기존 순서(Step 3 → 4 → 4.5 → 5) 그대로, 본 단계 이하의 분리 절차 diff 0.

**분리 적용 시 흐름 재배치**:

```
Step 2 (Requirements 합의)
→ Step 4.5 목업 (앞당김 — B 기동 전에 사용자 승인 완료. 미승인 목업 = 정답지 아님)
→ 분기: [B 서브에이전트 격리 호출 — Test Plan 출제 (Step 5-B)] ∥ [A: Step 3 Scope + Step 4 Dev Plan]
→ B 산출 수령 → 원문 독립 보존 → A 검토(방식 칸 한정) → Test Plan 섹션 반영
→ Step 6 spec-diff → Step 7 통합 합의 → planned
```

- 병렬 실행 가능한 플랫폼(클로드)은 B 기동 직후 A가 Step 3·4를 병렬 진행. 순차만 가능한 환경(코덱스)은 **B를 먼저 완료시킨 후** A가 Step 3·4를 작성한다.
- 어느 쪽이든 불변식은 같다: **B 기동 시점에 Dev Plan은 존재하지 않는다** — 구현 관점이 시험지에 스며드는 오염이 구조적으로 불가능하다.
...
> **분리 적용 태스크(Step 2.5)는 본 Step을 A가 작성하지 않는다** — Step 5-B(B 출제)로 수행하고, A는 산출 검토만 한다. 본 Step의 형식·규칙(방식 선택 / 시나리오 형식 / 커버리지 / 실행 경로 확인)은 B 펜스에 동일 계약으로 내장돼 있다. 분리 비적용 태스크만 본 Step을 직접 작성한다.
...
- **기대값 출처 표기 (UX/UI task 한정)** — 모든 `[AUTO]` PASS 기준 끝에 출처를 단다: `(출처: 목업 §N / Req N)`. 출처를 못 다는 기대값 = 승인 근거 없는 추정 — 숨기지 말고 `(출처 없음 — 사용자 확인 필요)`로 표기해 Step 7 합의 때 *사용자 확인 항목*으로 분리 제시한다. task-test가 문 앞 검사에서 출처 표기 유무를 확인한다(룰 11).
...
### Step 5-B — Test Plan 출제 분리 (분리 적용 태스크 한정 — Step 2.5)

Step 5를 A(메인)가 작성하는 대신, **격리 서브에이전트 B**가 Requirements 합의문 + 승인 목업 + `TEST_RULE.local.md` **만** 입력으로 Test Plan을 출제한다.

#### 1. B 기동 (서브에이전트 격리 호출)

prompt는 자기완결적 — 아래 펜스 그대로. 메인 컨텍스트(Scope·Dev Plan·코드 지식) 일체 안 들어감.

**B prompt 본문 (정확히 이 형식)**:

```
당신은 격리 세션의 Test Plan 출제자입니다. 이 prompt에 명시된 자료만 보고, 구현 계획·코드에 대한 어떤 가정도 없이 검증 시나리오를 출제합니다.
...
승인된 목업 경로 (있을 때만): <ABSOLUTE_PATH_TO_MOCKUP>
이 프로젝트의 검증 규칙 (있을 때만): <ABSOLUTE_PATH_TO_TEST_RULE_LOCAL_MD>

위 자료 외 참조 금지 — task 문서·코드·기타 문서를 열지 않습니다. Requirements에 적힌 것만 검증 대상입니다 (적히지 않은 요구를 추정해 시나리오를 만들지 않는다).
검증 규칙 파일이 경로로 주어졌으면 반드시 Read — ① 방식별 실행 경로(각 방식을 이 프로젝트에서 실제로 어떻게 돌리나) ② 테스트 실행 환경(점유 스위트 격리 경로·사용자 결정 기록) ③ 범위·방식 정책.

## 출제 룰

1. **시나리오 형식 = [실제 실행할 명령/입력] + [구체적·관측 가능한 기대값] 한 쌍 강제.** "정상 동작 / 잘 됨 / 확인한다" 같은 소원은 기대값이 아님. 장면 쪼개기 — 한 덩어리 금지, 진짜 떨어질 장면 여러 개로 분해.
2. **방식 선택은 기계적으로** — 화면 신규·변경=시각(캡처-목업 대조) / 클릭·입력 상호작용=UI 동작 / 엔드포인트=API 호출 / 데이터 쓰기·변경=사이드 이펙트 조회 / 순수 계산·변환=입출력 비교 / 기존 영역 접촉=회귀. 여러 개 걸리면 다 붙인다.
3. **방식 칸은 검증 규칙의 "방식별 실행 경로" 절에 등재된 경로로 채운다.** 필요한 방식의 등재 경로가 없으면 그 칸에 "등재 경로 없음 — 메인 확인 필요"라고 적는다 (실행 경로를 지어내지 않는다).
4. **어서션 정면성** — 기대값이 요구사항 자체를 관측해야 한다. "벗어날 수 없다"가 요구면 실제로 벗어나 보는 것이 정면이고, 버튼 disabled 확인 같은 우회 지표는 통로가 열려 있어도 통과하므로 PASS 기준이 될 수 없다.
5. **기대값 출처 표기** — UX/UI 요구의 모든 [AUTO] PASS 기준 끝에 출처를 단다: (출처: 목업 §N / Req N). 출처를 달 수 없는 기대값은 (출처 없음 — 사용자 확인 필요)로 표기한다 (추정 은폐 금지).
6. **화면·입력 점유 스위트**(XCUITest·headed 브라우저 E2E 등)는 검증 규칙 "테스트 실행 환경" 절의 *현재 기록* 기준으로 분류한다 — 격리 실행 경로 선언 있음=[AUTO] / "없음 — 매번 승인" 기록 또는 무기록=시나리오에 "사용자 승인 후 실행" 표기. 사용자에게 물을 수 없으므로 질문이 필요한 판단은 메인 몫으로 남긴다.
7. **grep/Read-only 존재 확인 시나리오 = 보조용** — 단독 PASS 가치 없음. Requirements 각 항목당 end-to-end 실동작 시나리오 최소 1개.
...
| # | 시나리오 | 분류 | 방식 | PASS 기준 (구체적 기대값) |
|---|---------|------|------|----------|
| 1 | <쪼갠 장면 한 줄> | [AUTO] / [USER] | <등재 경로 기반 방식> | <관측 가능한 구체값 (출처: ...)> |
...
위 형식 그대로 리턴 후 종료. 코드·문서 수정 금지 — 출제만.
```

**구현 디테일**:
- `<REQUIREMENTS_INLINE>` 자리에 Step 2에서 합의된 `## Requirements` 섹션 **전문을 그대로 인라인** (경로 아님 — B가 task 문서를 열지 않게).
- 목업 경로는 Step 4.5 승인 완료본이 있을 때만, `TEST_RULE.local.md` 경로는 파일 실재 시만 삽입 — 없으면 그 줄 자체를 제외한다 (task-test Step 3과 동일 방식).
- **코덱스 spawn (플랫폼 패리티)**: `.codex/agents/test-plan-author.toml`의 `test-plan-author` 서브에이전트로 spawn + 위 펜스 전체를 그대로 전달 (task-test의 `task-tester`와 동일 패턴 — 펜스가 양 플랫폼 공통 단일 룰 소스).

#### 2. B 원문 독립 보존

B 산출을 받으면 **그대로** `.project/tasks/<NNN_slug>/<NNN>_<slug>_test-plan-origin.md`에 Write한다 — 수정 금지. 위치는 plan 폴더 공통(spec-diffs·mockup과 동일 — 폴더 승격 무관, `TASK_DOC_RULE` §1.5). task-test 시험지 오염 대조(룰 18)의 기준 사본이며, closed 잠금 밖의 역사 자료다.

#### 3. A 검토 (수정권 = 방식 칸만) + 반영

- A의 수정권 = Test Plan 표 **방식 칸만**, 그것도 `TEST_RULE.local.md` 등재 경로로 교체할 때만. **시나리오 칸·PASS 기준 칸은 불가침.**
- 교체한 칸에 플래그 3요소 강제: `【A수정 · 원안: "<B 원문>" · 근거: TEST_RULE.local.md §<절>】` — 무플래그 수정은 task-test가 시험지 오염으로 즉시 반려한다.
- A가 처리하지 않고 **사용자에게 보고**하는 3케이스:
  1. 등재 어느 경로로도 확인 불가 (도구 부재)
  2. 방식 교체가 PASS 기준 문구 변경을 수반
  3. B의 기대값이 A의 이해와 충돌하거나 터무니없음 — **해석 분기 신호**다. 고치지 말고 드러낸다 (드러내는 것이 분리의 목적).
- 검토 후 Test Plan 섹션을 task 문서에 반영. "사용자 승인 후 실행" 점유 스위트가 있고 그 원인이 *테스트 실행 환경* 절 무기록이면, Step 7 자리에서 출구 질문(카탈로그의 점유 동작 스위트 5단계 절차 4)을 사용자와 진행한다.
...
1. 작성된 4 섹션 보여주기 — *"이렇게 가자?"* 합의 요청. **분리 적용 태스크는 추가로**: B 원문(origin)과 반영본의 차이(A수정 플래그 칸)·Step 5-B의 사용자 보고 3케이스·`(출처 없음 — 사용자 확인 필요)` 기대값을 명시 제시하고, 출구 질문이 남아 있으면(점유 스위트 무기록) 여기서 1회 진행한다. **`(출처 없음)` 기대값을 사용자가 확인해 주면 그 칸의 출처를 `(출처: 사용자 확인 YYYY-MM-DD)`로 갱신한다** (origin과 달라지므로 `【사용자결정 · 원안: "…"】` 플래그 — 그대로 두면 task-test 문 앞 검사(룰 11)가 자격 미달로 반려한다).
2. 사용자 *"OK"* / *"수정해줘"* 응답:
   - **OK** → Step 8 진행.
   - **수정 요청** → 해당 섹션 수정 후 다시 합의. 한도 없이 반복 OK (반복 잦으면 task 분리/규모 재조정 검토).
   - **Test Plan 칸이 사용자 결정으로 바뀌면** (기대값 충돌 해소 등) 그 칸에 플래그를 단다: `【사용자결정 · 원안: "<원문>"】` — origin과 달라지는 정당 경로도 표기해야 task-test 오염 대조(무플래그 차이 = 무조건 오염)를 통과한다.
...
- 출제 분리: <적용 (B 출제 · origin 보존 · A수정 N칸) / 비적용>
- 상태: draft → planned
- Phase 수: <N>개 (규모: <X>)
- spec-diff: <있음 / 없음>
...
- **서브에이전트 격리 호출**: Step 5-B의 B 출제 (분리 적용 태스크 한정 — 코덱스는 `test-plan-author`)
- **Bash / 도구**: `set_status` 도구(또는 `npx @angar2/taskery set-status`) status 전이 (유효전이 코드 검증)
- **Write**: spec-diff 파일 (Phase 0 변경 시) / B 원문 보존 파일 (`<NNN>_<slug>_test-plan-origin.md`)
