...
`npx @angar2/taskery set-status TASK-<NNN> developing` — 진입 시 `planned`이면 `developing`으로 전이. 이미 `developing`(이어가기)이면 idempotent no-op. (유효전이 코드 검증.)
...
1. `npx @angar2/taskery set-status TASK-<NNN> developed` — self-check + 테스트 PASS 후 전이 (유효전이 코드 검증).
2. 결과 보고:
...
- **Bash**: 검증 명령 실행 (린트/타입/빌드/단위테스트) / `npx @angar2/taskery set-status` 호출(status 전이). **git 명령 절대 X — `/task-close`만 git 담당**
