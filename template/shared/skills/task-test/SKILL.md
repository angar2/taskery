...
|--------|--------------|---------------------|
| 등록 (`.project/`가 `.gitignore`에 있음) | `$MAIN_WT/.project/tasks/<NNN_slug>/...` | `$WT_PATH` (워크트리 — 코드 변경 위치) |
| 미등록 (**기본 설치값** — `fork`/`status` 반환의 `registered:false`) | `$WT_PATH/.project/tasks/<NNN_slug>/...` (워크트리 안) | `$WT_PATH` (워크트리) |

격리 세션은 *워크트리 안*에서 검증/테스트 명령 실행 (`$MAIN_WT/AGENTS.md`의 `## 검증 명령` / `## 테스트 명령` 단일 진실 소스).

**검증 방법 출처 = `$MAIN_WT/.project/rules/TEST_RULE.local.md`** — 각 테스트 방식(데이터 조회 / API 호출 / E2E / 시각 실행)을 *이 프로젝트에서 실제로 어떻게 돌리나*가 적힌 단일 소스. 격리 세션 prompt에 task 문서 경로와 *동일 방식*으로 이 절대 경로를 넘긴다 (메모리 아닌 파일 — 세션/컴팩트 무관 재사용).
...
5. **프로젝트 로컬 룰 확인** — `$MAIN_WT/.project/rules/TEST_RULE.local.md`를 Read한다. 이 파일은 모든 리포에 있어야 하는 필수 문서다(`TASKERY_RULE` §8) — **없으면 사용자에게 알리고** 이번 검증은 실행 경로를 지어내지 말고 진행한다(확인 불가한 시나리오는 실행 보류). 로컬 룰의 조항은 코어와 동등한 구속력을 가진다 — 겹치면 **로컬이 최우선**이고, **겹치지 않는 로컬 조항도 그대로 준수한다** (`TASKERY_RULE` §7). 단 멀티세션 불변식·destructive 승인·상태 전이 유효성은 로컬이 뒤집지 못한다.
...
이 프로젝트의 검증 규칙(있을 때만): <ABSOLUTE_PATH_TO_TEST_RULE_LOCAL_MD>
시험지 원본(있을 때만): <ABSOLUTE_PATH_TO_TEST_PLAN_ORIGIN_MD>

task 파일을 직접 Read 해서 ## Test Plan 섹션 + ## Dev Plan의 Phase별 완료 기준을 정독하세요. 위에 경로로 명시된 문서(task 파일 · 검증 규칙 · 시험지 원본) + 목업 외 다른 문서 참조 X.
(단 task.md가 `mockup/<task-doc-name>-mockup.html` 참조하면 해당 목업 파일도 직접 Read — *승인된 목업 = 시각 시나리오의 정답지*입니다. 시각 시나리오 = 실제 화면 캡처 → 이 목업과 자동 대조.)
검증 규칙 파일(`TEST_RULE.local.md`)이 경로로 주어졌으면 반드시 Read한다 — 두 가지가 그 안에 있다. ① *방식별 실행 경로*: 각 테스트 방식을 이 프로젝트에서 실제로 어떻게 돌리나(DB 조회 경로 / API 호출법 / E2E 셋업 / 시각 실행 / 격리 실행 환경). ② *범위·방식 정책*: 무엇을 얼마나 검증하나. 위 수행 룰과 겹치는 항목은 그쪽을 우선하고, 겹치지 않는 조항도 그대로 준수한다(멀티세션 불변식·안전 조항 제외). 경로가 주어지지 않았으면 그 프로젝트에 아직 규칙이 없다는 뜻이므로, 실행 경로를 스스로 지어내지 말고 확인 불가한 시나리오는 실행 보류로 리턴한다.
...
8. 코드 파일 직접 수정 절대 금지 (**유일 예외 = 룰 19 음성 대조의 일시 되돌림** — 즉시 원복 의무). 임시 파일(테스트 스크립트 등) 생성 시 종료 후 삭제.
...
11. **문 앞 검사 — 실행 전 시험문제 자격 확인 (먼저)**. 시나리오를 돌리기 *전에*, 각 `[AUTO]` 시나리오가 **[실행할 명령/입력] + [구체적 기대값(관측 가능한 결과 — 출력값 / 상태코드 / 어서션 / 조회 결과 / 화면 상태)]** 한 쌍을 갖췄는지 확인. *"정상 동작" / "잘 됨" / "확인한다"* 같은 *소원*은 기대값이 아님. **UX/UI task의 `[AUTO]` 기대값은 출처 표기(`(출처: 목업 §N / Req N)`)도 자격 요건** — 무표기 또는 `(출처 없음 — 사용자 확인 필요)` 잔존은 승인 근거 없는 추정이므로 자격 미달. 자격 미달 `[AUTO]` 시나리오가 있으면 → **그 시나리오는 검사하지 말고 "시험문제 결함(검증 불가)"으로 분류** + 무엇이 빠졌는지 지목 (아래 룰 13). 자격 미달을 PASS로 반올림 영구 금지.
...
14. **시각 `[AUTO]` 시나리오 = 캡처-목업 대조**. 앱을 띄워 실제 화면을 캡처 → 승인된 목업(정답지)과 대조. (본 룰은 TEST_RULE.local.md에 캡처 실행 경로가 있는 경우에만 적용된다. 경로가 없으면 해당 시나리오는 `[USER]`이며 룰 4 단서에 따른다.) 어긋나면 → *구체적 어긋남 목록*(예: "헤더 색 틀림 / 카드가 가로 아닌 세로로 쌓임 / 저장 버튼 없음")을 뽑아 **FAIL**로 보고. 합격선 = *픽셀 일치가 아니라* "사람이 보면 '이거네' 할 정도로 본질적으로 같다" (객관적 깨짐만 잡고 미세 취향은 룰 4로). 증거 = 스크린샷 + 어긋남 목록.
...
17. **화면·입력 점유 스위트 자동 실행 금지**. 실제 앱을 띄워 UI를 자동 조작하는 스위트(XCUITest·headed 브라우저 E2E 등)는 사용자의 화면·마우스·키보드를 점유해 작업을 중단시킨다. TEST_RULE.local.md *테스트 실행 환경* 절에 **격리 실행 경로**(창 숨김·헤드리스 플래그 / 컨테이너·가상 디스플레이 / VM / CI 러너)가 선언돼 있으면 그 경로로 실행한다. 선언이 없으면 — 비점유 부분만 한정 실행할 수 있으면 그것만 실행하고, 한정 수단도 없으면 **실행하지 말고** 해당 시나리오를 *실행 보류(사용자 승인 필요)*로 표시해 메인에 리턴한다 (무단 실행 금지). 헤드리스 모드가 있으면 항상 그쪽을 쓴다.

18. **시험지 오염 대조 (시험지 원본 경로가 주어졌을 때 — 먼저)**. 원본 파일을 Read → task.md `## Test Plan` 표와 *칸 단위*로 대조한다.
    - **무플래그 차이 = 무조건 오염** — 어느 칸이든 원본과 다른데 플래그(`【…】`)가 없으면 **시험지 오염**으로 즉시 반려.
    - **플래그 칸은 종류별 검증** — `【A수정 · 원안: "…" · 근거: TEST_RULE.local.md §…】`는 *방식 칸만* 허용(원안 문자열이 원본과 일치해야 하며, 시나리오·PASS 기준 칸의 A수정 플래그 = 오염). `【사용자결정 · 원안: "…"】`·`【보수 · 원안: "…"】`는 허용 경로 — 원안 표기가 원본과 일치하는지만 확인.
    - **방식 교체를 빙자한 침범** — A수정 플래그 칸에서 방식 외(기대값·시나리오 의미)가 함께 바뀌었으면 오염.
    - 오염 판정 = UNCERTAIN(검증 불가) 계열 + **"시험지 오염" 라벨**로 리턴 — 어느 칸이 어떻게 다른지 명시. 오염이 없으면 통상 검증을 계속한다.
19. **음성 대조 (핵심 시나리오 최소 1개 — 룰 8의 명시 예외)**. 핵심 시나리오 1개 이상은 구현 코드를 Edit로 *일시 되돌린* 상태에서 그 시나리오가 **실제로 FAIL함**을 확인한 후 즉시 원복하고, 그 사실을 통과 근거에 포함한다 — 테스트가 버그를 잡을 수 있음의 증명(통과만 보면 무력한 테스트를 모른다). git 명령 금지 — Edit 원복만. **종료 시 되돌림 흔적 0 의무** (원복 후 스스로 diff 확인). 되돌리기가 위험한 영역(마이그레이션·전역 상태·파괴적 사이드 이펙트)이면 생략 + 사유 기록.
...
### 시험지 오염 (UNCERTAIN 검증 불가 · 오염 라벨 — 룰 18 해당 시)
- <어느 칸이 원본과 어떻게 다른가 + 플래그 유무 + 원안↔현재 인용>
  → 메인이 원본 기준 복원 + 사용자 보고 필요.

### 음성 대조 결과 (룰 19 — 수행 시)
- <어느 시나리오를 어떤 되돌림으로 FAIL 확인했나 + 원복 확인(diff 무변) / 생략 시 사유>

### 실행 보류 (UNCERTAIN 실행 보류 — 해당 시)
- <어느 시나리오/명령을 실행하지 않았나 + 무엇을 점유하는가(화면·마우스·키보드·전역 단축키 등) + 비점유 부분만 실행했다면 그 범위>
  → 메인이 사용자 승인을 받아 실행하거나, TEST_RULE.local.md *테스트 실행 환경* 절에 격리 실행 경로를 마련한 뒤 재검증 필요.
...
- `<ABSOLUTE_PATH_TO_TEST_RULE_LOCAL_MD>` 자리에 `$MAIN_WT/.project/rules/TEST_RULE.local.md`의 절대 경로 삽입 (위 "멀티세션 메타 위치"의 `MAIN_WT` 사용 — 워크트리에서 호출돼도 메인 워크트리의 단일 소스를 읽도록). **파일이 없으면 그 줄 자체를 prompt에서 제외**한다 (없는 경로를 넘기면 격리 세션이 헤맨다). 0.7.0 이전 리포라 파일이 없고 구형 `$MAIN_WT/.project/TEST-GUIDE.md`가 남아 있으면 **그 경로를 대신 넘기고**, 메인은 사용자에게 `TEST_RULE.local.md`로의 이행을 1회 제안한다.
- `<ABSOLUTE_PATH_TO_TEST_PLAN_ORIGIN_MD>` 자리에 `.project/tasks/<NNN_slug>/<NNN>_<slug>_test-plan-origin.md`(출제 분리 태스크의 B 원문 — task-plan Step 5-B가 생성)의 절대 경로 삽입. **파일이 없으면 그 줄 자체를 prompt에서 제외**한다 (분리 비적용 태스크 — 룰 18 자체가 발동하지 않음).
- **메인 측 원복 안전망 (룰 19)**: 격리 세션 기동 *직전* 워크트리 스냅샷을 뜬다 — ① `git -C "$WT_PATH" diff` 출력 해시 ② `git -C "$WT_PATH" status --porcelain -uall` 목록 ③ 미추적 파일 각각의 내용 해시. 리턴 *직후* 같은 방식으로 재계산해 대조하고, 불일치(격리 세션의 되돌림 미원복·잔재) 시 사용자 보고. ①만으로는 부족하다 — task-dev 시점 변경분은 미커밋이라 *신규 파일*이 `git diff`에 안 잡힌다(②③까지 합산해야 원복 실패를 잡는다).
...
4. **"시험지 오염" 라벨(룰 18)인 경우** — Test Plan 보수가 아니라 **메인이 origin(`<NNN>_<slug>_test-plan-origin.md`) 기준으로 task.md Test Plan을 복원**한 후 `/task-test` 재실행. 사용자 보고에 귀책을 명시한다: 오염 칸에 플래그 有 → A 수정 귀책 / 결함이 origin 그대로의 칸이면 → B 출제 품질 (원안의 사후 귀책 평가). 무플래그 오염은 발생 자체가 규약 위반이므로 경위를 함께 보고한다.
...
```
   단, `TEST_RULE.local.md` *테스트 실행 환경* 절에 *"없음 — 매번 승인(사용자 결정 …)"*이 이미 기록돼 있으면 선택지 3("격리 경로 만들자")을 새 제안처럼 내지 않는다 — 이미 내린 결정이다. 승인 여부(1·2·4)만 묻는다.
3. 사용자 응답별 처리:
   ...
   - **"격리 경로 만들자"** → 경로(창 숨김·헤드리스 / 컨테이너 / VM / CI)를 마련하고 `TEST_RULE.local.md` *테스트 실행 환경* 절에 기록한 뒤 `/task-test` 재실행. 다음 task부터는 승인 없이 자동 실행된다.
...
**조건**: ① 종합 판정 PASS + 이번 세션이 여기서 멈추고 사용자에게 제어를 넘김(= `/task-close`로 바로 진행하지 않음) + ② `AGENTS.md`에 `## 검수 실행 명령`이 선언됨.
