...
`npx @angar2/taskery set-status TASK-<NNN> testing` — 진입 시 `developed`이면 `testing`으로 전이. 이미 `testing`(이어가기)이면 idempotent no-op. (유효전이 코드 검증.)
...
task 파일을 직접 Read 해서 ## Test Plan 섹션 + ## Dev Plan의 Phase별 완료 기준을 정독하세요. 그리고 *검증 방법 문서(TEST-GUIDE.md)* 도 직접 Read — 각 테스트 방식을 *이 프로젝트에서 실제로 어떻게 실행하나*(DB 조회 경로 / API 호출법 / E2E 셋업 등)의 단일 소스입니다. 이 둘 + 목업 외 다른 문서 참조 X.
...
4. `[USER]` 시나리오 — *정답지가 주관*인 영역만 (시각 미세 취향 / 색조·여백 2px 수준 / UX 사용감·매끄러움). UNCERTAIN(사람 검수)으로 분류 + *사용자 직접 검수 항목*으로 리턴 (단 증거=캡처 첨부). **시각 *객관* 영역(레이아웃 깨짐 / 요소 빠짐 / 빈 화면 / 색 완전히 틀림 / 배치 딴판)은 `[USER]`가 아니라 `[AUTO]`** — 실제 화면 캡처 → 승인 목업과 직접 대조해 격리 세션이 판정. (시각이라고 무조건 사람에게 넘기지 말 것 — 객관 깨짐은 네가 잡는다.)
...
14. **시각 `[AUTO]` 시나리오 = 캡처-목업 대조**. 앱을 띄워 실제 화면을 캡처 → 승인된 목업(정답지)과 대조. 어긋나면 → *구체적 어긋남 목록*(예: "헤더 색 틀림 / 카드가 가로 아닌 세로로 쌓임 / 저장 버튼 없음")을 뽑아 **FAIL**로 보고. 합격선 = *픽셀 일치가 아니라* "사람이 보면 '이거네' 할 정도로 본질적으로 같다" (객관적 깨짐만 잡고 미세 취향은 룰 4로). 증거 = 스크린샷 + 어긋남 목록.

## 결과 형식 (이 포맷 그대로 리턴)

### 종합 판정
- **PASS** / **FAIL** / **UNCERTAIN** 중 하나
- **UNCERTAIN** = 아래 둘 중 하나라도 1개 이상이면. *두 경우 모두 PASS 자동 전이 X* (공통). 그 다음 종류에 따라 라우팅이 갈림:
...
- 종합이 UNCERTAIN일 때 *두 종류 중 무엇인지* 반드시 표기 (둘 다면 둘 다).
- 한 줄 요약
...
1. `npx @angar2/taskery set-status TASK-<NNN> tested` (PASS 전이 — 유효전이 코드 검증).
2. 결과 보고:
```
✅ TASK-<NNN> 격리 검증 PASS
- 시나리오 <N>개 모두 PASS
...
```
3. 사용자 *"고쳐"* → `npx @angar2/taskery set-status TASK-<NNN> developing` + `/task-dev` 안내. 사용자 *"OK 마무리"* → `npx @angar2/taskery set-status TASK-<NNN> tested` (Result 섹션에 *"알려진 결함 있음"* 명시) + `/task-close` 안내.
...
- **Edit**: Result 섹션 기록
- **Bash**: `npx @angar2/taskery set-status` 호출 (status 전이 — 유효전이 코드 검증)
...
> **세 갈래 요약**: PASS → `tested` / 코드 결함 → `developing`(사용자 *"고쳐"*) / 시험문제 결함 → Test Plan 보수(`task-plan`, 코드·status 보존) → 재검사. UNCERTAIN(사람 검수)은 검수 ✓ 받으면 `tested`, ✗면 `developing`.
