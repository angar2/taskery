...
- **상황**: 셋 중 하나라도 마찰로 누적될 때 —
  - (a) **brownfield 중간 도입**: 이미 진행 중인 프로젝트에 taskery를 중간 설치. 선제 기획(project-init/plan-init) 없이 출발해야 하는데, 9 기획 문서를 *사후에* 작성하면 코드를 움직이지 않는 허구 문서가 되고 콜드스타트 비용이 큼.
...
- **상태**: 미래 옵션이 아니라 *적용 대기열 2순위*(싸고 안전, 빠른 청소).
- **출처**: 본 대화(2026-06-26) — 원안 #4. plan의 두 축 겸함 진단 + "폴더는 사실 검색 인덱스였다"는 정체 규명을 거쳐 "대부분 코드 불필요"로 좁혀짐.
